...
export function CasosSidebar() {
  ...
  return (
    <>
      ...
      <aside
        className="hidden md:flex w-60 shrink-0 flex-col sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto bg-white border-r border-[#a8dbde]"
      >
      ...
      <button
        className="fixed top-[60px] left-4 z-50 md:hidden w-10 h-10 flex items-center justify-center rounded-lg bg-white border border-[#a8dbde] shadow-sm"
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            {/* Sidebar overlay */}
            <motion.aside
              className="fixed top-0 left-0 h-full w-64 z-50 flex flex-col bg-white border-r border-[#a8dbde] md:hidden"
              initial={{ x: -256 }}
              animate={{ x: 0 }}
              exit={{ x: -256 }}
