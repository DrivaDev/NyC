...
| Heading | 20px | text-xl | 600 | font-semibold | 1.3 | Subtitulo de login "Candidaturas Val Thorens"; header de la card del formulario |
| Body | 16px | text-base | 400 | font-normal | 1.5 | Labels de campos, texto de ayuda, contenido general |
| Label/Small | 14px | text-sm | 500 | font-medium | 1.4 | Labels sobre inputs, texto de error inline, nombre de archivo subido |

**Regla de pesos:** Solo se usan font-bold (700) para Display y font-semibold (600) para Heading. Body y Label usan font-normal (400) y font-medium (500) respectivamente. Nunca font-light ni font-extrabold.
...
| Accent (10%) | #0055A4 | bg-blue-700 (custom) | Solo: boton primario "Enviar candidatura"; borde del drag-and-drop zone en estado active/hover; indicador de foco en inputs |
...
Accent reservado para: boton submit principal, borde del dropzone en hover/drag-active, ring de foco de inputs (focus:ring-french-blue). NUNCA usar el azul como color de fondo de pagina, texto de body, o decoracion.
...
2. Subtitulo "Candidaturas Val Thorens" — `text-xl font-semibold text-gray-500` — centrado, mt-2
...
**Header de la card:**
- "The Annex" en `text-xl font-semibold text-gray-900`
...
- Label: "Nombre completo" — `text-sm font-medium text-gray-700`
...
- Label: "Curriculum Vitae (PDF)" — `text-sm font-medium text-gray-700`
...
- Estado con archivo subido: reemplaza el dropzone con una fila `flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-gray-200`
  ...
  - Nombre del archivo — `text-sm font-medium text-gray-700 truncate`
...
- Label: "Tipo de trabajo" — `text-sm font-medium text-gray-700`
...
- Label: "Idiomas que hablas" — `text-sm font-medium text-gray-700`
...
- Label: "Disponibilidad" — `text-sm font-medium text-gray-700`
...
- Texto: "Enviar candidatura" — `text-base font-semibold`
